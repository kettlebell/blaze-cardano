...
async function main() {
  ...
  const [blaze, pubKey] = await getBlaze();
...
function buildNFTDetails(
  ...
  const toMint: [Core.Script, string][] = [];
  const NUM_NFTS = 7;
...
async function mintTokens(asset_name: string, quantity: bigint) {
  ...
  const scriptPK = new Core.ScriptPubkey(Core.Ed25519KeyHashHex(""));
  const before = new Core.TimelockExpiry(Core.Slot(123));
  ...
  const tx = await blaze.newTransaction()
    ...
    .addMint(policyId, new Map().set(unit, quantity), Data.void())
    .setValidUntil(Core.Slot(69321696 + 1000))
    .complete();
